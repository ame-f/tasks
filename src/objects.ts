...
export function isCorrect(question: Question, answer: string): boolean {
    let answer_real = answer.trim();
    answer_real = answer_real.toLowerCase();

    let expected_real = question.expected.trim();
    expected_real = expected_real.toLowerCase();
    if (answer_real == expected_real) {
        return true;
    }
...
export function renameQuestion(question: Question, newName: string): Question {
    const up_q = { ...question, name: newName };
    return up_q;
}
...
export function publishQuestion(question: Question): Question {
    const new_q = { ...question, published: !question.published };
    return new_q;
}
